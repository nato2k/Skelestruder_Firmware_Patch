...
if [ $# -lt 2 ]
  then
    num1=840
	num2=16
	echo "default steps selected 840/16"
  else
    num1=$1
	num2=$2
...
python3 bmg_patch.py $num1 $num2
rc=$?
if [ $rc = 1 ]
then
	...
	results=$(./Prusa-Firmware/PF-build.sh 1_75mm_MK3S-EINSy10a-E3Dv6full.h EN_ONLY GOLD)
	...
	cp $fwfull complete_builds/$fwnew-BMGMK3S-E$num1-u$num2-$(date +%Y%m%d).hex
	echo "compiled file is located at: $(pwd)/complete_builds/$fwnew-MK3S-$(date +%Y%m%d).hex"
	echo "complete!"
fi
